<div class="max-w-2xl flex gap-12 p-6 border border-[#252525] rounded-2xl h-56 mx-auto">
  <div class="min-w-[200px] h-full flex justify-center items-center bg-white rounded-xl">
    <img src="https://www.refract-agency.com/favicon.png" alt="Logo" class="w-36 h-36">
  </div>

  <div class="mt-4 md:my-0">
    <div class="flex items-center flex-wrap mb-4 md:mb-1.5 gap-2">
        <h3 class="inline-block text-2xl font-medium">Refract Agency</h3>
        <span class="tag-l">Marketing Agency</span>
    </div>
    <div>
        <p class="text-[16px] font-[430] brightness-150 mt-1">
            My design, development + visualisation studio for creative businesses and ambitious
            startups.
        </p>
    </div>
    <div class="flex gap-2 mt-4">
        <button on:click={() => window.open('https://aarkyv.com/studio', '_blank')}
            >Visit Website</button
        >
        <button
            class="bg-transparent border border-[--color-accent] text-[--color-accent]"
            on:click={() => (window.location.href = 'mailto:hello@aarkyv.com')}>Contact Us</button
        >
    </div>
</div>
</div>
